...
def packet_to_dict(pkt: bytes) -> dict:
    ...
    if int(pkt_split[1][5:]) == PKT_EXEC:  # If the received packet is for executing a command by itself
        ...
        ret = {'type': int(pkt_split[1][5:]), 'ack_num': int(pkt_split[2][8:]), 'files': int(pkt_split[3][6:]),
               'cmd_length': int(pkt_split[4][8:]), 'command': pkt_split[5][4:], }
        ...
        # Convert packet to dictionary
        ret = {'type': int(pkt_split[1][5:]), 'ack_num': int(pkt_split[2][8:]), 'files': int(pkt_split[3][6:]),
               'output_length': int(pkt_split[4][14:]), 'output': pkt_split[5][7:], }
        ...
        # Convert packet to dictionary
        ret = {'type': int(pkt_split[1][5:]), 'ack_num': int(pkt_split[2][8:]), 'filesize': int(pkt_split[3][9:]),
               'filename': pkt_split[4][9:], }
        ...
        raise ValueError(
            "Command type of packet not recognised: type={}, from={}".format(int(pkt_split[1][9:]), hosts[0]))
...
def execute():
    ...
    for i in range(len(actionsets)):
        ...
        for j in range(len(curr_actset)):
            ...
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            if host_used == -1:
                sock.connect(('127.0.0.1', int(def_port)))
            else:
            ...
            packet = "{{ type:{} ack_num:{} files:{} cmd_len:{} cmd:{} }}".format(cmd_type, ack_num, cmd_files, cmd_len,
                                                                                  text_to_bits(curr_cmd["cmd"]))
            ...
            if cmd_files > 0:
                ...
                while i < cmd_files:
                    ...
                    file_details_pkt = "{{ type:{} ack_num:{} filesize:{} filename:{} }}".format(PKT_FILE_DETAILS,
                                                                                                 ack_num, file_size,
                                                                                                 text_to_bits(
                                                                                                     file_path))
                    ...
                    if pkt['type'] == PKT_TERMINAL_OUTPUT:
                        ...
                        if pkt['files'] > 0:
                            ...
                            while files_received < pkt['files']:
                                ...
                                # Send ack
                                sock.send(
                                    "{{ type:{} ack_num:{} }}".format(PKT_ACK, file_details_packet['ack_num']).encode(
                                        'utf-8'))
                                ...
                                while new_file_data:
                                    ...
                                    if os.path.getsize(file_details_packet['filename']) != file_details_packet[
                                        'filesize']:
                                        print(
                                            "!!!!!!!!!! Error file received was not the size given !!!!!!!!!!, path={}, given size={}, actual size={}".format(
                                                file_details_packet['filename'], file_details_packet['filesize'],
                                                os.path.getsize(file_details_packet['filename'])))
                                        sys.exit(1)
                                    break
